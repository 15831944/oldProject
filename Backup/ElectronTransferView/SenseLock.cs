...
{
    internal class SenseLock:Singleton<SenseLockImplement>
    {

    }
    internal class SenseLockImplement:IDisposable
    {
        //Sense4 API
        ...
        {
            uint ret = S4ChangeDir(ref si[0], "\\");
            var pinBytes = Encoding.ASCII.GetBytes(pin);
            return 0==S4VerifyPin(ref si[0], pinBytes, (uint)pinBytes.Length, S4_USER_PIN);
        }
        ...
        public string GetKey() 
        {
            ...
            if (0 == S4Execute(ref si[0], "581a", null, 0, outBuffer, 32, ref BytesReturn))
            {
        ...
        public DateTime GetTime()
        {
            uint BytesReturn = 0;
            byte[] outBuffer = new byte[32];
            if (0 == S4Execute(ref si[0], "3456", null, 0, outBuffer, 32, ref BytesReturn))
            {
        ...
        public void Open() 
        {
            try
            {
                uint ret = S4Open(ref si[0]);
                IsOpened = 0x00 == ret;
                Debug.Assert(0x00 == ret);
            }catch
            {
                MessageBox.Show("请检查加密狗是否已插好！", "系统提示", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                //Register.RemoveDemandLoadingEntries("CustomAutoCAD", false);
            }
        }

        public void Close() 
        {
            uint ret = S4Close(ref si[0]);
            if (IsOpened)
            {
                IsOpened = 0x00 == ret;
            }
